...
function MealsPage() {
  ...
  const range = safeProteinRange(s.profile);
  ...
  const save = () => {
    ...
    if (protein < range.min) {
      msg = `You've had about ${protein} grams of protein today. Try to reach at least ${range.min} grams — add some dal, paneer, eggs or curd.`;
    } else if (protein > range.max) {
      msg = `You've had about ${protein} grams of protein today. That's above your safe range of ${range.max} grams — please reduce protein-heavy foods tomorrow, and drink more water.`;
    } else {
      msg = `Wonderful! ${protein} grams of protein today, right within your safe range of ${range.min} to ${range.max} grams.`;
    }
  ...
  const proteinStatus =
    protein < range.min ? "low" : protein > range.max ? "high" : "ok";
